...
const update = (() => {
    ...
    return (previous, next) => {
        ...
        const isAvailable = feeds.reduce((p, c) => p.concat(c.xml.entries), [])
        ...
        if (isAvailable) { // Even if the selected entry is available still the parent might have been changed
            selected.parent = feeds.filter(o => o.xml.link === selected.parent.xml.link)[0];
        } else {
            // does the old account still have unread entries?
            const obj = feeds.filter(o => selected.parent && o.xml.link === selected.parent.xml.link);
            if (obj.length && obj[0].xml.fullcount) {
                ...
                selected.parent = feeds.reduce((p, c) => c.xml.fullcount ? c : p);
        ...
        if (doAccountBody) {
            ...
            const messageID = gmail.get.thread(selected.entry.link);
            stat.current = index + 1;
...
const labels = url => {
    ...
    if (o) {
        ...
        parent.textContent = '';
        o.labels.map(s => s === '^i' ? 'Inbox' : s).filter(s => s.startsWith('^') === false && skippedLabels.indexOf(s) === -1).forEach(label => {
...
const fetchLabels = () => {
    let url = selected.entry.link;
    document.body.dataset.labels = "false";
    document.body.dataset.star = 'hide';

    const query = 'in:' + (selected.parent.xml.label || 'inbox') + ' is:unread';

    gmail.labels({url, query}).then(r => {
        if (!r) {
...
new Listen('accounts', 'click', ({target}) => {
    ...
    if (selected.parent.xml.link !== link) {
        const obj = feeds.reduce((p, c) => c.xml.link === link ? c : p);
...
// init, get cached object and showing first message
qs('iframe').onload = () => chrome.storage.session.get({'cached-objects': []}, prefs => {
    feeds = prefs['cached-objects'];

    if (feeds && feeds.length) {
        // Selected account
        const unreadEntries = feeds.map(obj => obj.xml.entries
        ...
        if (unreadEntries.length) {
            ...
            selected.parent = feeds.reduce((p, c) => c.xml.entries.indexOf(newestEntry) !== -1 ? c : p);
        }
        if (!selected.entry) {
